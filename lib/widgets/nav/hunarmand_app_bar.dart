...
///   - READS FROM: providers/dynamic_content_provider.dart → logoPath
...
class HunarmandAppBar extends StatelessWidget implements PreferredSizeWidget {
    ...
    return AppBar(
      ...
      title: Center(
        ...
        child: ConstrainedBox(
          ...
          constraints:
              const BoxConstraints(maxWidth: AppBarUIConfig.maxContentWidth),
          child: Padding(
            // Standard horizontal padding from design tokens (24px)
            padding: const EdgeInsets.symmetric(
                horizontal: AppBarUIConfig.screenPadding),
            child: Row(
              children: [
                // ── LEFT: Branding Section ──
                // Shows a logo image sourced from DynamicContentProvider
                ...
                if (isDesktop) ...[
                  ...
                  const SizedBox(
                      width:
                          AppBarUIConfig.spacerMedium), // 16px gap before CTAs
                ] else ...[
                  // Mobile/Tablet: Only the two most important links
                  // Other links are accessible via HunarmandDrawer
                  _navItem(
                      context, 'Courses', 'courses'), // Priority link: courses
                  _navItem(context, 'About', 'about'), // Priority link: about
                  const SizedBox(
                      width: AppBarUIConfig.spacerSmall), // 8px gap before CTAs
                ],
                ...
                const SizedBox(
                    width:
                        AppBarUIConfig.spacerSmall), // 8px gap between buttons
  ...
  Widget _buildBranding(BuildContext context, bool isDesktop) {
    return Consumer<DynamicContentProvider>(
      builder: (context, provider, _) {
        final logoPath = provider.content.logoPath;
  ...
  Widget _buildAssetLogo(bool isDesktop) {
    return Image.asset(
      'assets/images/main_logo.png',
      height: isDesktop
          ? AppBarUIConfig.iconSizeLarge + 16
          : AppBarUIConfig.iconSizeLarge + 4,
      fit: BoxFit.contain,
    );
  }
  ...
  Widget _navItem(BuildContext context, String label, String page) {
    ...
    return MouseRegion(
      cursor: SystemMouseCursors
          .click, // Pointer cursor indicates clickable element
      child: GestureDetector(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(
            horizontal:
                AppBarUIConfig.spacerSmall + 2, // 10px horizontal padding
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(
            horizontal:
                AppBarUIConfig.spacerMedium - 2, // 14px horizontal padding
          ...
          child: Row(
            children: [
              ...
              const SizedBox(
                  width: AppBarUIConfig.spacerSmall /
